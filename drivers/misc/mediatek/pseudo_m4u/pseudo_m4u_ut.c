...
struct test_buffer_info *m4u_test_start(size_t size, int port)
{
	...
	if (IS_ERR(handle)) {
		pr_err("Fatal Error, ion_alloc for size %d failed\n",
			     size);
		goto err;
	}
	...
	if (ion_kernel_ioctl(client, ION_CMD_MULTIMEDIA,
		...
		pr_err("m4u_test_drv: Config buffer failed.\n");
		goto err;
	}

	ion_phys(client, handle, &buffer_mva, &mva_size);
	if (buffer_mva == 0) {
		pr_err("Fatal Error, get mva failed\n");
		goto err;
	}
