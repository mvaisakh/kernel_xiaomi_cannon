				     ...
				     const char *buf, size_t count)
{
	...
	queue_delayed_work(system_power_efficient_wq,
		&haptic->load_waveform_work, 0); //delay 10s
	return count;
}
...
static DEVICE_ATTR(activate, 0644, aw8622_activate_show,
		   aw8622_activate_store);
static DEVICE_ATTR(index, 0644, aw8622_index_show,
		   aw8622_index_store);
static DEVICE_ATTR(duration, 0644, aw8622_duration_show,
		   aw8622_duration_store);

static DEVICE_ATTR(hwen, 0644, aw8622_hwen_show,
		   aw8622_hwen_store);

static DEVICE_ATTR(load_wavefile_ctrl, 0644, aw8622_load_wavefile_ctrl_show,
		   aw8622_load_wavefile_ctrl_store);

static DEVICE_ATTR(debug_val_ctrl, 0644, aw8622_debug_val_ctrl_show,
		   aw8622_debug_val_ctrl_store);

/* sys debug node */
static DEVICE_ATTR(pwm_ctrl, 0644, aw8622_debug_pwm_ctrl_show,
		   aw8622_debug_pwm_ctrl_store);
static DEVICE_ATTR(test_cnt, 0644, NULL,
...
static int aw8622_haptic_probe(struct platform_device *pdev)
{
	...
	queue_delayed_work(system_power_efficient_wq,
		&haptic->load_waveform_work, 10 * HZ); //delay 10s
