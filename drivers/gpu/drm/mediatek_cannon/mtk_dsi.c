...
static enum DISPPARAM_MODE g_last_hbm_level = DISPPARAM_LCD_HBM_OFF;
static bool g_thermal_hbm_control = false;
...
#if 1
	if (DISPPARAM_THERMAL_HBM_ENABLE == cmd) {
		pr_info("%s-%d:enable hbm cmd\n", __func__, __LINE__);
		g_thermal_hbm_control = false;
		if (dsi->output_en) {
			// restore hbm level
			cmd = g_last_hbm_level;
		}
		is_from_thermal = true;
	} else if (DISPPARAM_THERMAL_HBM_DISABLE == cmd) {
		pr_info("%s-%d:disable hbm cmd\n", __func__, __LINE__);
		g_thermal_hbm_control = true;
		if (!dsi->output_en) {
			pr_info("%s-%d: skip disable hbm cmd\n", __func__, __LINE__);
			mutex_unlock(&dsi->dsi_lock);
			return 0;
		}
		cmd = DISPPARAM_LCD_HBM_OFF;
		is_from_thermal = true;
	}

	if (!is_from_thermal) {
		if (DISPPARAM_LCD_HBM_L1_ON == cmd || DISPPARAM_LCD_HBM_L2_ON == cmd
			|| DISPPARAM_LCD_HBM_L3_ON == cmd || DISPPARAM_LCD_HBM_OFF == cmd) {
			// record hbm level
			g_last_hbm_level = (enum DISPPARAM_MODE)cmd;
			if (g_thermal_hbm_control) {
				pr_info("%s-%d:skip hbm cmd: 0x%x during thermal control\n",
