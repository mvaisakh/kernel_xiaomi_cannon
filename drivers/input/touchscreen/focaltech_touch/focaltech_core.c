...
void fod_dbg_print_info(struct fod_dbginfo *dbg)
{
	...
	MI_TOUCH_LOGD(1, "point: (%d, %d)", dbg->x, dbg->y);
...
static int fts_read_and_report_foddata(struct fts_ts_data *data)
{
	...
	if (ret < 0) {
		 ...
		 * */
		/*MI_TOUCH_LOGI(1, "FTS:read fod data: buf[2] = 0x%x", buf[2]);*/
		switch (buf[2]) {
		case 0x24:
...
#if FTS_GESTURE_EN
#ifdef CONFIG_TOUCHSCREEN_FTS_FOD
	ret = fts_read_and_report_foddata(data);
	if (ret == 0) {
		...
		if (data->suspended && data->fod_status <= 0) {
			MI_TOUCH_LOGI(1,"skip point report");
			return 1;
		}
	}
#else
...
static irqreturn_t fts_ts_interrupt(int irq, void *data)
{
	...
	us = 20 * 1000;
	pm_qos_update_request_timeout(&ts_data->tp_qos_request, 500, us);
#if FTS_ESDCHECK_EN
...
static void fts_update_touchmode_data(int mode)
{
	...
	case Touch_Edge_Filter:
			if (fts_data->gamemode_enabled) {
				...
				ret  = fts_i2c_read_reg(fts_data->client, FTS_REG_EDGE_FILTER_ORIENTATION, &current_filter_orientation);
				if (ret < 0)
					MI_TOUCH_LOGE(1, "read  edge filter direction  error, ret=%d", ret);
				if (0 != xiaomi_touch_interfaces.touch_mode[Touch_Edge_Filter][GET_CUR_VALUE]){
					if (0 == current_filter_orientation)
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pm_qos_add_request(&ts_data->tp_qos_request, PM_QOS_CPU_DMA_LATENCY,
