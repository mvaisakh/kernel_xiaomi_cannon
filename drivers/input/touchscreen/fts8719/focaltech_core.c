...
static void fts_irq_read_report(void)
{
	...
	if (ts_data->tp_pm_suspend) {
		ret = wait_for_completion_timeout(&ts_data->pm_resume_completion, msecs_to_jiffies(500));
		FTS_INFO("pm_resume_completion be completed, handling irq ret = %d\n", ret);
	}
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	init_completion(&ts_data->pm_resume_completion);
#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
...
static int fts_pm_suspend(struct device *dev)
{
	...
	reinit_completion(&ts_data->pm_resume_completion);
	ts_data->tp_pm_suspend = true;
...
static int fts_pm_resume(struct device *dev)
{
	...
	ts_data->tp_pm_suspend = false;
	complete(&ts_data->pm_resume_completion);
